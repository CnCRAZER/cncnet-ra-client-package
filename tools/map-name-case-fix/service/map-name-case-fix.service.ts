...
export class MapNameCaseFixService {
    ...
    private async loadMapDirectoryFilesAsync(): Promise<void> {
        this.mapDirectoryFiles = await this.readDirAsync(coreConstants.paths.raMaps);
    ...
    private getMapKeyImagePath(mapKey: string): string {
        const mapFileBase = path.join(coreConstants.paths.raMaps, mapKey.substring(mapKey.lastIndexOf('\\') + 1));
